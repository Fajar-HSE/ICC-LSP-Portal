export function pagerHTML(current: number, total: number, kind: string): string {
  ...
  html += `<button type="button" data-page="${current - 1}"${current <= 1 ? ' disabled' : ''} aria-label="Halaman sebelumnya">← Prev</button>`;
  ...
  html += `<button type="button" data-page="${current + 1}"${current >= total ? ' disabled' : ''} aria-label="Halaman berikutnya">Next →</button>`;
  html += '</div>';
  return html;
}
